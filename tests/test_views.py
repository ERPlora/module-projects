...
class TestProjectViews:
    ...
    def test_edit_form_loads(self, auth_client, project):
        """Test edit form loads."""
        url = reverse('projects:project_edit', args=[project.pk])
        response = auth_client.get(url)
        assert response.status_code == 200

    def test_edit_post(self, auth_client, project):
    ...
    def test_delete(self, auth_client, project):
        ...
        url = reverse('projects:project_delete', args=[project.pk])
        response = auth_client.post(url)
        assert response.status_code == 200
        project.refresh_from_db()
        assert project.is_deleted is True

    def test_bulk_delete(self, auth_client, project):
        """Test bulk delete."""
        url = reverse('projects:projects_bulk_action')
        response = auth_client.post(url, {'ids': str(project.pk), 'action': 'delete'})
        assert response.status_code == 200
        project.refresh_from_db()
        assert project.is_deleted is True
